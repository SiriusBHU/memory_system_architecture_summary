...
# ---------------------------------------------------------------- 配色
C_SYS    = "#a9ddb6"   # 内核 + 系统服务（底座，taupe）
C_APPS   = "#f6c49b"   # TOP 应用（Native + 三方，合并为一层，蓝）
C_CAP    = "#1a4f7a"   # 容量（深蓝）
...
sys_floor = np.array([2.7, 2.8, 2.9, 2.3, 2.1])          # 内核+系统服务：2026 见顶，随回退（2026→2027）瘦身
...
ax.text((PROJ_START + years[-1]) / 2, 13.6, "外推区（2026.7 — 2028）",
...
polys[0].set_alpha(0.3)   # 内核+系统服务（底座）
polys[1].set_alpha(0.3)   # TOP 应用
...
fig.savefig(os.path.join(HERE, "memory-capacity-crunch-lowend-conflict-kkk.png"), dpi=150)
fig.savefig(os.path.join(HERE, "memory-capacity-crunch-lowend-conflict-kkk.svg"))
