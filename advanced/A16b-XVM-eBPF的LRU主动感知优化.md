...
> **⚠️ 本篇立场（先读）**：本篇讨论的「在 eBPF/XVM 之上做面向 Agent 负载的整机 LRU 主动感知优化」是**基于学界研究与第一性原理的设计探讨与可行性分析**——**eBPF/XVM 作为可编程基础设施或已存在（XVM 据用户口径见于鸿蒙），但这套面向 Agent 负载的优化方案，业界（Android / iOS / HarmonyOS）尚未落地**。全篇严格区分**① 已有机制与学术工作**（如 eBPF struct_ops / eBPF-mm / DAMON）与**② 本篇设想**（非现状，标「设想 / 推测 / 待核实」）；XVM 的接口/版本因公开资料稀薄一律待核实，不臆造。核心问题：顺着第一性原理与学界，这样做**是否成立、可行性多大**。
...
> **把 LRU/回收路径上的"判决"开成一个可挂载自定义程序的钩子**——内核走到"这页要不要回收"时，回调一段外部下发的逻辑，由它说"保护 / 回收 / 用默认"。**据本系列口径，XVM 即 HarmonyOS 用 eBPF 实现这条思路的框架**（公开资料稀薄，其边界与本篇所述 Agent 感知优化的关系见 §3.3，一律待核实）。
