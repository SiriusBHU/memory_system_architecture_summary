...
> **⚠️ 本篇立场（先读）**：本篇的「用边际成本模型在延迟/吞吐预算内控制压缩激进度」是**基于学界研究与第一性原理的原创分析框架**，**不是任何已落地的内核模块**，业界（Android / iOS / HarmonyOS）现状只是用 `swappiness` 等静态参数近似。全篇严格区分**① 已有机制与学术工作**（如 `writeback_limit`/PSI、Adaptive Cache/Memory Compression 等）与**② 本篇推演的模型**（**全部公式均标"示意，非实测"**，量级需实测标定）。核心问题：顺着第一性原理与学界，这样建模**是否成立、可行性多大**。
