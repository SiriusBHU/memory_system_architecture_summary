...
> **⚠️ 本篇立场（先读）**：本篇讨论的「由 OS 协同管理 DRAM + PIM 两类异构介质」是**基于学界研究与第一性原理的设计探讨与可行性分析**，**业界（Android / iOS / HarmonyOS）目前并无 OS 层的 PIM 介质概念，更未落地此方案**。全篇严格区分**① 已有机制与学术工作**（事实，如 HBM-PIM/AiM 器件原型、LP-Spec 等研究，多为数据中心/原型）与**② 本篇设想**（非现状，标「设想 / 推测 / 待核实」）；器件性能数字均为厂商/论文声明、需独立复核。核心问题：顺着第一性原理与学界，这样做**是否成立、可行性多大**。
