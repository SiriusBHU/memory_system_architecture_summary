...
> **⚠️ 本篇立场（先读）**：本篇讨论的「用统一内存 + 异构 Page Fault / LRU 把设备（DMA）数据纳入冷热治理」是**基于学界研究与第一性原理的设计探讨与可行性分析**，**业界（Android / iOS / HarmonyOS）目前并未落地此方案**。全篇严格区分**① 已有机制与学术工作**（事实，如 SVA/ATS/PRI、HMM/`ZONE_DEVICE`，多在数据中心/Linux 上游）与**② 本篇设想**（非现状，标「设想 / 推测 / 待核实」）；下文「现状与平台差异」列的是 ① 的**已有基础设施**，**不代表该方案已被任何移动平台实现**。核心问题：顺着第一性原理与学界，这样做**是否成立、可行性多大**。
