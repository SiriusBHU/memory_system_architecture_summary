...
> **⚠️ 本篇立场（先读）**：本篇讨论的「在 Agent 负载下用主动扫描 / 主动回收取代『前台热·后台冷』的场景式冷热判定」是**基于学界研究与第一性原理的设计探讨与可行性分析**，**业界（Android / iOS / HarmonyOS）目前并未落地此方案**。全篇严格区分两层——**① 已有的通用机制与学术工作**（事实、带来源，如 MGLRU / DAMON / `memory.reclaim` / Senpai）与 **② 本篇据此推演的设想**（非现状，标「设想 / 推测 / 待核实」）；下文「现状与平台差异」列的是 ① 的**已有基础设施**，**不代表该方案已被任何平台实现**。核心问题始终是：顺着第一性原理与学界，这样做**是否成立、可行性多大**。
...
> **先纠一个最常见的误读**：上表"目的＝预腾挪"那一行，常被读成"主动回收能**降低内存峰值**"——这是错的。主动扫描**不降低工作负载本身的峰值需求**（突发该要多少还要多少），它只改变"同样的突发撞上来时会发生什么"：

```
被动 · 压力到达时才扫：
 上限 ┄┄┄┄┄┄┄┄✗┄┄┄┄┄┄┄┄┄┄
            ╱╲              ← 撞顶：direct reclaim 卡顿 / lmkd 杀
    ____╱      ╲______
  └────────────────────→ 时间   （占用贴着需求爬，headroom ≈ 0）

主动 · 压力之前就腾出空隙：
 上限 ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄   ← 始终不触顶
      ░░░░░░░░░░░░░░░░░░     ← 预留 headroom（提前腾出）
   __/\__╱╲__/\______       ← 后台慢扫压低占用；突发撞进 headroom 被吸收
  └────────────────────→ 时间   （代价：持续扫描耗 CPU/电/闪存）
```

| 维度 | 削的是什么 | 代价 |
|---|---|---|
| 被动 | —（撞顶才被动救急） | 低：不扫不耗，但尾延迟 / 被杀不可控 |
| 主动 | **峰值时刻的同步阻塞（direct reclaim 卡顿）、lmkd 被杀率、尾延迟**——靠"提前腾出的 headroom"，**不是把占用压低** | **持续后台扫描耗 CPU/电/闪存**（可能多压、压错还要 refault 读回） |

一句话：**"削峰"要精确说成"削峰值时刻的阻塞与被杀概率"，而非"削峰值内存需求"**；而这份收益**本身要花能耗**——这正是 §3.3 / §6 那个"能耗感知反馈回路"存在的理由：没有把激进度钳在续航 / 闪存预算内的闭环，削峰就是白烧电、磨闪存，得不偿失。
...
主动化**不是一条单线**，而是沿**两条正交轴**同时往右上角爬——横轴管 **何时扫·谁握触发权**，纵轴管 **判冷热多便宜·扫描器**：

| 判冷热 ↓ ＼ 何时扫 → | 被动·撞水位才扫 | 命令式·「回收 N」 | 声明式·目标 + 反馈 |
|---|---|---|---|
| **区域采样**（DAMON） | — | DAMON_RECLAIM（固定方案） | **DAMOS aim-oriented 自调 ★前沿** |
| **页表走查**（MGLRU） | MGLRU aging（默认开） | `lru_gen` debugfs 主动产代 / 回收 | — |
| **rmap 反查**（经典 LRU） | 经典回收 `kswapd` / 直接回收 | `memory.reclaim`、`/proc/<pid>/reclaim`、`process_madvise` † | Senpai / TMO † |

- **纵轴向上＝判冷热越便宜**（扫描成本与内存规模解耦，即 [A05](../foundations/A05-冷热识别的演进.md) 那条线）；**横轴向右＝时机越提前、节奏从"撞水位"→"周期"→"自适应"，触发权从内核 → userspace 命令 → 目标闭环**。"何时扫"与"谁握触发权"在这条轴上是同一件事：最左的"被动"已把"晚时机（压力到了才扫）"编码进去了。
- **† 命令式 / 声明式接口本身不绑定扫描器**：底下用的是当前启用的 LRU——双链则走 rmap、MGLRU 则走页表；列在此行只表示它们最初 / 默认建立在经典 LRU 之上。
- **★ 右上角＝双轴顶点**（实测采样 + 目标闭环）是终端最该去、却**尚未落地**的角（目标须并入续航 / 闪存，见 §6）。关键区分：**Senpai/TMO 与 DAMOS 都到了横轴最右端（都是反馈回路），但 Senpai 底下仍走 LRU（纵轴只到中段），DAMOS 才换上 DAMON 实测采样（纵轴到顶）**——这是单线列表说不清、二维坐标才摆得清的事。

> 注：本表所列均为**已有的通用机制 / 学术工作（①）**，仅"★ 双轴顶点在终端的落地"属本篇**设想（②）**，与全篇立场（§开篇）一致。

把这张坐标按时间拉直，就是下面这条演进线（每一步都在推某条轴往右上角爬）：
...
| 主动扫描基础设施 | MGLRU `min_ttl_ms` 稳定可用；`lru_gen` debugfs 与 DAMON `pageout`/`cold` 可做主动回收 | 无 MGLRU/DAMON 对应物；XNU 自有 pageout 扫描器 | 待核实（[A14-HarmonyOS](../platforms/A14-HarmonyOS-内存实现.md) 无相关公开细节）；可编程回收方向见 [A16b eBPF 可编程回收](A16b-eBPF可编程回收策略-Android.md) |
...
- **激进度的自调节是主旋律，但目标函数在终端要改写**：Senpai/DAMOS 的反馈回路在数据中心以"PSI / 内存占用"为目标；终端要把**续航与闪存寿命**也并进目标（多压一页省了 RAM，却烧了 CPU、磨了闪存）。**"何时值得花一次扫描 + 一次压缩去腾一页"在能耗约束下仍无公开的可移植基线（待核实）**，与 [A16d 压缩 IP 边际建模](A16d-压缩IP边际建模.md) 是同一问题的两面。另注：主动回收的**收益是削"峰值时刻的阻塞与被杀"、并非削"峰值内存需求"**（详见 §3 纠偏），所以"削峰"与"耗能"本是同一枚硬币，反馈回路就是那台天平。
- **扫描的能耗本身要被建模**：主动扫描越频繁，冷热视图越新，但 aging/采样自身耗电。终端理想是**按压力与能耗预算自适应调扫描频率**，而非固定节奏——目前是厂商各自调参。
- **谁来驱动尚无定论**：内核内（DAMON_RECLAIM 自调）还是用户态（Senpai 式 daemon）？终端上更可能是**一个感知前后台、感知 agent 生命周期的 userspace 策略** + 可编程内核钩子的组合——这正引向 [A16b 的 eBPF 可编程回收策略](A16b-eBPF可编程回收策略-Android.md)。
...
| 可编程化 → eBPF | 把"按什么策略判冷热/回收"做成可下放的程序 | **[A16b](A16b-eBPF可编程回收策略-Android.md)** |
...
**承接 / 相邻篇**
- [A04 回收总论](../foundations/A04-回收总论.md)、[A05 冷热识别的演进](../foundations/A05-冷热识别的演进.md)、[A08 压力与低内存终止](../foundations/A08-压力与低内存终止.md)、[A15b DAMON 与分层内存实践](A15b-DAMON与分层内存实践.md)、[A15c 移动端分层内存与内存压缩前沿](A15c-移动端分层内存与内存压缩前沿.md)、[A16b eBPF 可编程回收策略](A16b-eBPF可编程回收策略-Android.md)、[A16d 压缩 IP 边际建模](A16d-压缩IP边际建模.md)
