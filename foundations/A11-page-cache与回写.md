...
> **待核实 / 待补**：f2fs / ext4 对 large folios for file 的支持版本与现状；`ondemand_readahead()` 是否已更名为 `page_cache_ra_order()`（二手来源所述，未在内核源码逐版核对）；HarmonyOS 文件缓存的内核实现；XNU UBC 与 APFS 回写策略的细节（留待 A14 平台对照）。
