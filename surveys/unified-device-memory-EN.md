...
- **Local source mirrors are now populated via WebFetch** (not raw HTML — the WebFetch tool returns a model-extracted Markdown rendering). Saved files under [`sources/unified-device-memory/`](sources/unified-device-memory/): SVA kernel doc (12 KB, full body), HMM kernel doc (9 KB, full body), LWN SVA explainer (2 KB — *abridged restatement, not full prose*; use the original URL for citation), and an `android-dmabuf-heaps.md` substitute (6 KB) covering the same ION→DMA-BUF heaps transition because the AOSP SPA at `implement-dma-buf-gpu-mem` consistently returned the Graphics overview body to the fetcher. For canonical text on any reference, prefer the original URL.

## 9. References

1. Linux kernel project. (2024). *Shared Virtual Addressing (SVA) with ENQCMD*. [docs.kernel.org/arch/x86/sva.html](https://docs.kernel.org/arch/x86/sva.html). Local copy: [`sources/unified-device-memory/sva-kernel-doc.md`](sources/unified-device-memory/sva-kernel-doc.md). — "SVA doesn't require pinning pages for DMA"; PASID/ATS/PRI dependencies.
2. Linux kernel project / Jérôme Glisse et al. (2019). *Heterogeneous Memory Management (HMM)*. [www.kernel.org/doc/html/v5.0/vm/hmm.html](https://www.kernel.org/doc/html/v5.0/vm/hmm.html). Local copy: [`sources/unified-device-memory/hmm-kernel-doc.md`](sources/unified-device-memory/hmm-kernel-doc.md). — `ZONE_DEVICE`, bidirectional migration, `mmu_notifier`.
...
5. Corbet, J. (2018). *Shared Virtual Addressing for the IOMMU*. LWN. [lwn.net/Articles/747230/](https://lwn.net/Articles/747230/). Local copy: [`sources/unified-device-memory/lwn-sva-iommu.md`](sources/unified-device-memory/lwn-sva-iommu.md) — *abridged via WebFetch; original URL is authoritative*. — Cross-arch SVA API explainer.
