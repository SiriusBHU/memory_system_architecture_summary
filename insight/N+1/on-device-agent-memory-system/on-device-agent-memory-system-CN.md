...
> 本文研究终端设备上长程 Agent 的**记忆系统**如何设计，所处层次比裸 KV Cache 存储高一层。对比的两端是：原始方案——Agent 的记忆就是当前上下文窗口（及其 KV Cache）；演进方案——一套外置、分层、自管理的记忆系统（"记忆 OS"），把活跃工作集压到很小，同时把长期记忆持久化、按需检索。本文是 [on-device-kv-cache-management](../on-device-kv-cache-management/on-device-kv-cache-management-CN.md) 的姊妹篇，后者覆盖位于本文之下的 KV Cache 存储层。
...
把前文反复引用的 MemoryOS（EMNLP 2025 Oral）拎出来逐层拆解——它是「外置分层记忆系统」这一思路最公开、可验证的真实系统。
