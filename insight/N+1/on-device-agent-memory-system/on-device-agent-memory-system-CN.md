...
上面几张表给了结构与规则，但没把动作走一遍。用一条具体事实——「用户对花生过敏」——把**写入（机制怎么运作）**和**读取（怎么匹配）**各追一遍。

![一条记忆的一生 + 一次命中](assets/memory-lifecycle.svg)

*图. 左：写入路径，一条记忆自下而上从问答沉淀到永久画像；右：读取路径，新问题三级召回后合并生成。复现脚本：[assets/memory-lifecycle.py](assets/memory-lifecycle.py)。*

**写入：一条记忆的一生（自下而上沉淀晋升）**

> Day1 10:00　用户：「我对花生过敏，帮我规划下周的食谱」　助理：「好的，已避开花生……」

1. **打包成 Dialogue Page，压入 STM。** 这一轮被打包成一页 `{Q, R, T, meta_chain}`（最小单元是一问一答，不是 token），压进 STM 的 7 页 FIFO 队列。STM 就是当前摆在窗口里的工作集，对应 KV Cache——无论历史攒了几个月，这里永远最多 7 页，有界。
2. **STM 满 → 最旧页挤进 MTM。** 第 8 轮进来时最旧页被弹出、迁入 MTM。它的向量与各 Segment 的中心比较：**余弦相似度 > θ=0.6 就并入该段**（这里并入「饮食/体质」段），否则新起一段。
3. **Heat 累积 → 超 τ=5 换页晋升 LPM。** 「饮食」段攒到 `Heat = N_visit 2 + 段内页数 3 + R_recency 0.99 ≈ 5.99 > τ=5`，触发晋升。注意 `R_recency = exp(−Δt/1e7)` 只落在 0–1（时间常数约 116 天），所以真正决定晋升的是「被检索次数 + 段内页数」，recency 只是「久未触碰就变冷」的衰减项。晋升后，「对花生过敏」定居进 LPM 的 User KB，STM/MTM 再换血也不丢。

**读取：一次命中（三级召回，不重放全历史）**

> Day30　用户：「周末家庭聚餐，推荐几道菜」

把问题编码成向量，三层各用各的方式召回：

- **STM**：不做匹配，最近 7 页整块带上（直近上下文）。
- **MTM**：两阶段——① 问题向量 × 各段中心 → 取 top-m=5 段；② 在这 5 段内 × 各页 → 取 top-k=5–10 页。命中那页「不爱吃香菜」。
- **LPM**：每类（User KB / Traits / Agent Persona）各取 top-10 语义匹配。命中「对花生过敏」。

Generation 把三层召回合并注入 LLM → 「（避开花生、少放香菜）推荐 ○○、△△」。不重放约 9K token 的全历史，只用命中的几页 + 画像几行，这就是 token 省 70–90%、p95 延迟降 91%、精度反升的来源。

**匹配的核心，两处别混。** 本质都是向量余弦相似度，但 **θ=0.6 是写入时**判「这页并入旧段还是新建段」的阈值；**top-m / top-k 是读取时**「先段后页、两级收窄」的排序。即 MemoryOS「用阈值归簇，用排序收窄」。

**对照 Mem0 看「匹配」的另一种实现。** Mem0 没有分层 / FIFO / Heat：写入由 LLM 抽取事实后对既有记忆下 ADD/UPDATE/DELETE/NOOP（过敏信息变了就 UPDATE，重复就 NOOP），读取从单一向量库纯按语义近邻取 top-k。同是外置、检索式记忆，MemoryOS 是 OS 式分层 + 热度换页，Mem0 是扁平向量库 + LLM 整合。

### 这管的是哪类记忆？两条正交的分类轴

把「记忆」分类时常见的混乱，是把**载体**（存在哪、什么形态）和**内容**（关于什么的记忆）揉成一类——例如把「system prompt」「语义记忆」「带 KV cache 的那种」并列。其实它们分属两条**正交**的轴：

- **轴 A＝表示 / 载体**：`参数(weights)` ／ `明文·外置(flash·向量库)` ／ `激活·KV Cache(DRAM 活跃状态)`。这是 MemOS `MemCube` 提的三型记忆。
- **轴 B＝内容 / 功能**：`工作记忆` ／ `情景 episodic` ／ `语义 semantic(含画像)` ／ `程序性 procedural(技能·指令)`。这是认知科学的记忆分类。

KV Cache 是**轴 A 的一种载体**（激活记忆），不是内容类型；system / skill prompt 是**轴 B 的程序性·指令**，且以静态形态存在——两者都不该归到「语义记忆」。把元素摆进矩阵就一目了然：

![记忆分类的两条正交轴](assets/memory-taxonomy.svg)

*图. 横＝轴 A（载体：参数 / 明文外置 / 激活 KV），纵＝轴 B（内容：工作 / 情景 / 语义 / 程序性）。绿色虚线框＝本篇（MemoryOS/Mem0）管理范围；框外为静态·参数·指令。复现脚本：[assets/memory-taxonomy.py](assets/memory-taxonomy.py)。*

| 轴 B（内容）＼ 轴 A（载体） | 参数（权重，静态） | 明文·外置（flash·向量库，持久可检索） | 激活·KV Cache（DRAM，工作态） |
|---|---|---|---|
| **工作记忆 Working** | — | — | **STM**（7 页 FIFO，窗口工作集） |
| **情景 Episodic** | — | **MTM**（话题段，对话事件历史） | （STM 生页亦属情景） |
| **语义 / 画像 Semantic** | 模型权重内的世界常识（预训练固化） | **LPM 画像 / Mem0 事实库**（剥离上下文的事实） | ← MemOS：热点明文提升为激活注入 |
| **程序性 / 指令 Procedural** | 微调 / Adapter 固化的能力 | **system / skill prompt 原文**（静态指令） | **system/skill prompt 的 KV 前缀缓存**（prefix reuse，姊妹篇） |

**逐格读关键单元：**

- **STM**＝工作记忆 × 激活：当前摆在窗口里、由 KV Cache 支撑的有界工作集（7 页）。它处在工作记忆与情景的边界——装的是直近的生对话页。
- **MTM**＝情景 × 明文：按话题归并的对话事件历史，存在外置库里。这是典型的**情景记忆**。
- **LPM 画像 / Mem0 事实库**＝语义 × 明文：剥离了「何时何地说的」的事实与特性（如"对花生过敏"），存在外置库里。这是**语义记忆**（用户画像是其特例）。
- **模型权重内常识**＝语义/程序性 × 参数：预训练固化进权重的知识与能力，静态、不随会话改变。
- **system / skill prompt**＝程序性·指令 × 明文(原文)＋激活(KV 前缀)：它是「怎么表现、怎么解题」的**程序性指令**，以静态文本写就，载入推理后成为 KV 前缀（可前缀复用）。**它不是语义记忆，也不归本篇管理。**

**本篇管理范围（绿框）＝动态记忆里的「情景 → 语义/画像」固化管线。** 即矩阵中 STM（激活前端）＋ MTM（情景/明文）＋ LPM（语义/明文）这一块。Mem0 同理（抽取 salient facts ＝语义化）。框外的两类——`参数列`（权重常识、微调能力）与`程序性行`（system/skill prompt 及其 KV 前缀复用）——都是**静态**的，不由记忆 OS 在运行时写入/淘汰；其中 KV 前缀复用属于姊妹篇 [on-device-kv-cache-management](../on-device-kv-cache-management/on-device-kv-cache-management-CN.md) 的 KV 层。

**写入＝沿矩阵迁移。** 上一节「一条记忆的一生」对应图中的两步迁移：先从 STM（激活/工作）沉淀到 MTM（明文/情景），再固化到 LPM（明文/语义）——**这是沿轴 B「情景→语义」的固化，载体始终在外置侧**。而 MemOS 的「热点明文→激活注入」是**沿轴 A** 把语义记忆从明文提升为激活（KV）形态，正好接到下一节的 KV 同构。

一句话归位：

> 你直觉里「带 KV cache 那种」指的是**轴 A 的激活记忆**（载体），不是轴 B 的语义记忆（内容）；system/skill prompt 是**轴 B 的静态程序性指令**。本篇真正管理的，是动态记忆中**情景→语义**的固化与检索（外置为主、STM 为激活前端），而非静态/参数/指令。
