...
## 9. Case study: the internal structure of vLLM PagedAttention

This section dissects vLLM PagedAttention — the system that recurs throughout the "evolved" architecture in Section 4 — layer by layer, as the most public, verifiable real implementation of "KV cache management."
...
*Figure. PagedAttention's three-column structure: ① a request's logical blocks → ② block-table mapping → ③ physical KV block pool; the bottom shows the three mechanisms (on-demand allocation / copy-on-write sharing / automatic prefix caching). Reproduction script: [assets/pagedattention-arch.py](assets/pagedattention-arch.py).*
...
> Note: the table reports vLLM's original throughput/memory figures to characterize the **paging mechanism itself**; vLLM targets datacenter GPUs, so the on-device side does not inherit its throughput multipliers directly. The structural waste reduction (60–80%→<4%) is a hardware-independent mechanism gain that also holds on-device.
>
> On-device mapping: llama.cpp's `llama_kv_cache` (cells + seq_id sets + `llama_kv_cache_seq_cp` prefix sharing + defragmentation) is the mobile-side implementation of the same idea, corresponding to MemOS's **activation memory** layer [llama.cpp].
> This case (the KV/activation layer) and the upper-layer "memory system" case MemoryOS (the semantic/persona layer) are isomorphic across one memory hierarchy, joined by MemOS's `MemCube`; the structural side-by-side appears in the companion doc [on-device-agent-memory-system](../on-device-agent-memory-system/on-device-agent-memory-system-EN.md), Section 9.
...
3. **KVNAND** — Authors, 2025. "KVNAND: Efficient On-Device Large Language Model Inference Using DRAM-Free In-Flash Computing." arxiv 2512.03608. URL: https://arxiv.org/abs/2512.03608. Local copy: [sources/KVNAND-2512.03608.pdf](sources/KVNAND-2512.03608.pdf)
...
13. **vLLM / PagedAttention** — Kwon et al., 2023. "Efficient Memory Management for Large Language Model Serving with PagedAttention." SOSP 2023. Blog: https://vllm.ai/blog/2023-06-20-vllm ; prefix-caching design: https://docs.vllm.ai/en/latest/design/prefix_caching/
14. **llama.cpp KV cache (on-device mapping)** — ggml-org/llama.cpp. URL: https://github.com/ggml-org/llama.cpp
