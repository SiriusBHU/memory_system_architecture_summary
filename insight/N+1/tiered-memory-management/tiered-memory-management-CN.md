...
> 本文对比端侧设备（手机、平板、PC、车机、边缘板）有限 RAM 的内存管理：原始方案（纯 DRAM 单层 + 通用全局回收 + 朴素换页，压力下靠 OOM 杀进程）与演进方案（显式分级：DRAM ↔ 压缩内存 zram/zswap ↔ Flash/UFS 交换，由访问感知的冷热放置——MGLRU、降级/预取——驱动）。来源覆盖 2022–2026 内核社区（MGLRU、PSI、LMKD）、学界（DAC'24、ATC'25、FAST'25、MobiCom'26）与业界（Apple 内存压缩、Samsung RAM Plus、华为 HyperSpace Memory）。

## 1. 范围与方法

**领域定义。** 资源受限终端设备上，操作系统如何把一块**容量固定**的物理 RAM 分摊到多个层级：DRAM（快、贵、容量小）↔ 压缩内存（zram/zswap，在 RAM 内以 ~2:1 压缩存放冷匿名页）↔ Flash/UFS 交换（慢、廉、容量大）。核心问题是：在固定 RAM 预算下，如何判断每个页面的冷热并放到正确层级，使活跃工作集常驻 DRAM、冷页下沉到压缩或 Flash 层，从而在不杀进程的前提下扛住内存压力。

**"原始"与"演进"的含义。** *原始方案*是**纯 DRAM 单层 + 通用全局回收**：经典的 active/inactive 两链表 LRU + kswapd 扫描，换页朴素（直接写 Flash 或干脆不开 swap），没有真正的分级；一旦压力上来，低内存杀进程（LMK）直接干掉后台应用，把"热"应用打回冷启动。*演进方案*是**显式的端侧分级**：DRAM ↔ 压缩内存（zram/zswap、Android RAM Plus、iOS/macOS 内存压缩器、华为 HyperSpace Memory）↔ Flash/UFS 交换，由访问感知的冷热放置驱动——MGLRU 多代际淘汰、按热度降级、预测式预取——让工作集保持常驻、冷页沉到压缩或 Flash 层。

**来源。** 13 个主要来源：6 篇学术论文（DAC'24、ATC'25、FAST'25/arXiv、MobiCom'26 等），4 份内核 / 系统资料（MGLRU 内核文档、Android LMKD、PSI、zram 文档），3 份业界资料（Apple WKdm 内存压缩、Samsung RAM Plus、华为 HyperSpace Memory）。来源类型涵盖同行评审系统论文、内核文档与厂商工程资料。其中 3 篇已保存本地副本。

## 2. 问题背景

**系统需要做什么。** 在 4–16 GB DRAM 的设备上同时维持几十个后台应用"存活"（avoid 冷启动），并在前台拉起 GB 级大应用（端侧 LLM、相机、地图、游戏）时立刻腾出内存。用户的硬指标是：切回后台应用要"秒开"，拉起大应用不能卡顿，且不能莫名其妙把后台应用杀掉。

**为什么这个领域变得困难。** 端侧 RAM **容量固定**、不能像服务器那样加内存条，但应用内存需求逐年膨胀（端侧 LLM 单个就要 GB 级）。冷启动代价极高：进程创建占冷启动总延迟的 **94%** [Ariadne]，所以系统必须尽量把应用以"存活"状态留在内存里。但留得越多，DRAM 越紧，回收和换页就越频繁——而 Flash 比 DRAM 慢两个数量级，朴素换页会直接拖垮交互延迟，于是早期 Android 干脆少用 swap、多杀进程。

**为什么原始方案不再够用。** 纯 DRAM 单层 + 两链表 LRU 在压力下只有两条出路：要么频繁触发 kswapd / 直接回收（direct reclaim）阻塞前台，要么靠 LMK 杀后台应用。前者在 Android 上是串行的、慢；后者把"热"应用打回冷启动。测量显示：**86.6%** 的 GB 级冷启动超过 1 秒这一"可用性悬崖" [AppFlow]；而把冷页放进压缩内存这条层级一旦缺失，要么浪费 DRAM，要么被迫写 Flash，二者都不优。

## 3. 具体问题与瓶颈证据

1. **纯 DRAM + 两链表 LRU 冷热判别太粗，错杀后台应用** — active/inactive 只有两档，无法细粒度区分"刚用过"与"很久没用"，导致 kswapd 既扫得多又杀得错。MGLRU 用多代际列表替换后，在 Google 车队上 kswapd CPU 占用降 **40%**、低内存杀进程降 **85%**（75 分位）、应用启动快 **18%**（50 分位）[MGLRU]。

2. **Android 串行回收阻塞前台响应** — 原始内核把页面"收缩"（shrinking）与"写回"（writeback）串行执行，前台拉起应用时被回收路径卡住。PMR 把二者解耦并行（PPS + SPW）后，应用响应时间最高改善 **43.6%** [PMR]。

3. **朴素换页：要么浪费 DRAM，要么硬写 Flash** — 不开 zram 则冷匿名页白占 DRAM；直接换到 Flash 则换入慢（解压远快于读 NAND）。zram 的存在本身就是分级：但静态、不区分冷热的 zram 既费 CPU 又费内存。Ariadne 指出朴素 zram "不区分冷热数据、不利用不同压缩块大小"，造成频繁无谓的压缩/解压 [Ariadne]。

4. **压缩内存的 CPU/内存成本与压缩率权衡** — zram 在 RAM 内换得越多，省的 DRAM 越多，但压缩/解压烧的 CPU 也越多；lz4 快但压缩率低（~2.6:1），zstd 压缩率高（~3.4:1）但更耗 CPU [zram]。Apple WKdm 把页面压到约一半（~2:1），代价是 CPU，但"比读写磁盘还快" [Apple]。

5. **压力下没有提前量，只能事后杀进程** — 缺乏预测式预取/回收时，系统只能等压力到了再回收，往往太晚而触发 LMK。AppFlow 用预测式预加载 + 自适应回收 + 上下文感知杀进程，把内核直接回收降 **67.9%**、LMK 事件降 **33.7%**、后台应用常驻数提升 **1.85×** [AppFlow]。
...
*原始方案：所有页面整页驻留单一 DRAM 层，两链表 LRU 冷热只分两档；内存压力下要么朴素换 Flash（慢），要么 LMK 直接杀后台应用，把"热"应用打回冷启动。*
...
*演进方案：三级层次（DRAM → 压缩内存 → Flash），由 MGLRU 多代际冷热判别 + PSI/LMKD 提前回收 + 冷热自适应压缩 + 预测式预取/批量写回共同驱动；活跃工作集常驻 DRAM，冷页逐级下沉而非被杀。新增/变更部分以 `*` 标记。*
...
- **串行回收阻塞前台** — PMR 把页面收缩与写回解耦并行（PPS），并对受害页批量解映射做存储友好写回（SPW），应用响应最高改善 43.6% [PMR]。AppFlow 的自适应回收按压力解耦 file-backed 与匿名页回收，单这一项贡献 60% 启动时间下降 [AppFlow]。

- **朴素换页浪费 DRAM 或硬写 Flash** — 压缩内存层（zram/zswap）以 ~2:1 在 RAM 内存冷页，解压远快于读 Flash；Ariadne 的冷热自适应压缩把应用重启延迟降 50%、压缩 CPU 降 15% [Ariadne]，ElasticZRAM 在 Pixel 6 上把响应时间改善最高 24.8% [ElasticZRAM]。华为 HyperSpace Memory 软硬协同把压缩率提升 69%、保活率提升 100%（"16GB≈20GB 体验"）[华为]。

- **压力下没有提前量** — AppFlow 用文件访问可预测性做预测式预加载（贡献 26.1%）+ 上下文感知杀进程（按净释放内存 ΔM 选受害者，杀进程少 30%），整体把 GB 级大应用冷启动从 2s 降到 690ms（−66.5%），95% 启动落在 1s 内 [AppFlow]。

### 尚未解决的问题

- **压缩内存吃 CPU、抢功耗预算** — zram/zswap 把内存压力转成 CPU 压力；持续压缩/解压在电池设备上与散热、续航冲突。Apple WKdm 选 ~2:1 低比率换速度正是这个取舍，但移动 SoC 上"压多少才划算"缺乏公开的功耗曲线。

- **Flash 仍比 DRAM 慢两个数量级，且有写寿命** — 第 2 层 UFS/eMMC 换入仍是硬延迟底线（解压 zram 远快于读 NAND）；持续换页还会消耗 Flash 的有限 P/E 擦写寿命，端侧的磨损均衡分析鲜有公开。

- **预测式预取在多任务下会失准** — AppFlow 的预测依赖"每个应用文件访问模式可预测"，但重度多任务、突发负载下预测命中率下降，错误预取反而浪费 I/O 与内存。

- **各家分级策略私有、不可移植** — Samsung RAM Plus、华为 HyperSpace Memory、Apple 内存压缩器各用私有实现与参数；同一套阈值换台设备（不同 DRAM/UFS 等级）就未必最优。
...
- **压缩内存的功耗预算未量化** — zram/zswap 把内存压力转成持续 CPU 开销，但移动/车机电池设备上"压缩省下的 DRAM"与"压缩多花的功耗/发热"之间的盈亏点缺乏公开实测。
- **Flash 换页的写寿命与磨损均衡** — 消费级 UFS/eMMC 擦写寿命有限，持续换页加速老化；端侧分级研究普遍只报延迟，不报磨损。
- **预测式调度在重度多任务下的鲁棒性** — AppFlow 的预加载/回收依赖访问模式可预测，突发负载下命中率与误预取代价尚需更广验证。
- **冷热引擎与上层 LLM KV Cache 交换的协同** — 端侧 LLM 自己也在换 KV Cache（见姊妹篇），它运行在内核 zram/swap 之外；二者各自分级、互不感知，可能重复换页或互相打架。
- **跨厂商 / 跨设备可移植性** — RAM Plus、HyperSpace Memory、iOS 压缩器参数私有，针对一款 DRAM/UFS 调的阈值换设备未必最优，缺乏统一抽象。
- **安全与隐私** — 换出到 Flash 的匿名页可能含敏感状态；端侧 swap 文件的加密与安全擦除鲜有公开工作。
