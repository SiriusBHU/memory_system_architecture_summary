...
> 本文对比端侧（手机、平板、笔电、边缘开发板）操作系统内存管理中的**原始方案**（固定、内建、一刀切的内核策略：全有全无 THP、固定 LMKD/oom_adj 阈值、静态 kswapd 水位）与**演进方案**（可编程、访问感知、随负载自适应的策略：多尺寸 THP「mTHP」、DAMON/DAMOS 访问监控驱动的回收与大页提升、可编程回收/压缩阈值，以及作为机制的 eBPF 风格挂钩）。调研覆盖 2021–2026 年内核社区（LWN、LPC、LSFMMBPF）与学界（MobiCom、SOSP、USENIX ATC、arXiv）的进展，并以 Android 上 mTHP 运行 2 小时后分配成功率从约 50% 跌至不足 10% 这一真实现象为锚点。

## 1. 范围与方法

**领域定义。** 资源受限终端设备上**操作系统级内存管理策略的可编程化与自适应化**——具体涉及大页（huge page / large folio）分配、页回收、低内存查杀（LMKD）、压缩交换（zRAM）阈值。重点不是某个具体应用，而是 OS 如何从「固定内建策略」转向「按设备 / 按应用 / 按前后台状态可调」的策略。

**"原始"与"演进"的含义。** *原始方案*是编译进内核、系统级生效的固定策略集：透明大页只有 `always`/`madvise`/`never` 三挡且贪婪提升 2 MB、LMKD 用固定 PSI/oom_adj 阈值杀进程、kswapd 用静态水位回收、zRAM 不区分冷热一律压缩。*演进方案*是一组让这些策略面变得**可编程、访问感知、自适应**的技术——多尺寸 THP（mTHP，按区域选 16 KB–2 MB）、DAMON/DAMOS 用低开销采样驱动回收与大页提升、热度感知的 zRAM 压缩、以及 eBPF 风格挂钩作为「运行时可定制内存策略」的机制。

**来源。** 14 个主要来源：内核社区报道与会议（LWN mTHP/DAMON 报道、LPC 2024 OPPO large folios、LPC 2024 可编程 MM、OSS NA 2025 Self-Driving DAMON）、端侧系统论文（MobiCom 2023 SWAM、MobiCom 2026 AppFlow、arXiv 2025 Ariadne）、机制类工作（eBPF-mm、cachebpf、FetchBPF、PageFlex），以及 Android 官方文档（LMKD/PSI）。来源类型涵盖内核维护者报道、同行评审移动系统论文、会议幻灯片与官方文档。机制类工作多源于服务器，本文仅作为「策略可编程」的实现手段引用，并在正文一句话标注其服务器出身后转向端侧映射。

## 2. 问题背景

**系统需要做什么。** 手机 / 边缘 OS 在每次缺页和每次内存压力事件上都要决定：分配多大的页（4 KB / 16 KB / 64 KB / 2 MB）、回收哪些页、何时杀哪个后台进程、哪些数据压进 zRAM——而设备只有 4–16 GB 物理内存，且要在前台交互（60/120 Hz，每帧 8.3–16.7 ms 预算）下保持不卡顿。

**为什么这个领域很难。** 与服务器不同，端侧没有 TB 级 DRAM 可挥霍，内存常年处于高压；设备会**连续运行数小时甚至数天**，物理内存随之严重碎片化。大页能减少 TLB 缺失、提升能效，但代价是：2 MB 页内部碎片浪费内存，碎片化后分配会触发同步压缩停顿；而手机长时间运行恰恰会把连续物理内存耗尽。固定的 LMKD 阈值要么杀得太晚（卡顿）、要么杀得太早（重启 App 冷启动慢）。

**为什么原始方案不再够用。** Android 上的 mTHP 控制仍是**系统级**的（经 sysfs），无法按设备实时碎片状态或按应用前后台状态调整；运行 2 小时后分配成功率从约 50% 跌至不足 10%，且「内核尚未达到可以自动使用 mTHP 的程度」[LWN-mTHP]。固定 LMKD 在内存压力下把后台 App 按序杀掉，导致频繁冷启动重载；传统 swap+OOMK 组合的 OOM 查杀次数可比自适应方案高 6.5 倍 [SWAM]。这些都说明：策略必须能在**运行时、按设备/负载**被改写，而不是编译时写死。

## 3. 具体问题与瓶颈证据

1. **全有全无 / 系统级 THP 在长时间运行后失效** —— mTHP（Linux 6.8+）虽引入 16 KB–512 KB 中间尺寸，但开关仍是系统级。Android 设备运行 1 小时后 mTHP 分配成功率约 50%，运行 **2 小时后失败率超过 90%**，内存完全碎片化、大页不再可得 [LWN-mTHP, OPPO-LPC]。

2. **碎片化触发的分配停顿与回收无访问感知** —— 固定 kswapd 水位与同步压缩在碎片化下放大延迟；缺乏访问热度信息时，回收会误伤热页。DAMON 用每 5 ms 采样 70 GB、**不到 1% 单核 CPU** 的开销提供访问热度，驱动的 proactive reclaim 在 ZRAM 上实测**节省 32% 内存、运行时开销仅 1.91%** [DAMON-Reclaim, DAMON-LWN]。固定策略拿不到这种信息。
...
4. **zRAM 不区分冷热、一刀切压缩拖慢相关重启** —— Android zRAM 对所有匿名页用同一压缩策略，是相关 App 重启慢、CPU 高的主因。热度感知、尺寸自适应的 Ariadne 把**相关重启延迟降低 50%**、压缩/解压 **CPU 降低 15%**（对比 SOTA zRAM）[Ariadne]。

5. **大 App 冷启动缺乏可编程的预取/回收协同** —— GB 级端侧 App（含本地 LLM/VLM）冷启动时，约 97.2% 的 I/O 是可预测的静态数据，却有 79% 的 DRAM I/O 带宽闲置。AppFlow 用内存调度把**冷启动延迟最多降低 66.5%**（2 s→690 ms）、内核直接回收减少 67.9%、LMK 事件减少 33.7% [AppFlow]。
...
*原始方案：所有策略编译进内核、系统级生效。THP 系统级且贪婪提升，回收/zRAM/查杀阈值固定，不区分应用前后台。长时间运行后碎片化使大页失效、查杀错位。*
...
*演进方案：在缺页、分配、回收/zRAM/查杀路径上插入可编程、访问感知的策略点（新增/变更以 `*` 标记）。用户态管理器用 DAMON 低开销采样获取访问热度、并感知应用前后台状态，按设备/负载下发策略或加载 eBPF；mTHP 按区域选尺寸、TAO 用保留区与双 LRU 维持大页可得性，DAMOS/Ariadne/SWAM/AppFlow 让回收、压缩、查杀、预取随负载自适应。*

## 5. 演进方案的收益与未解决问题

### 演进方案为什么有效

- **系统级 THP 长时间运行后失效** —— mTHP 按**区域**选 16 KB–2 MB 而非系统级一刀切；TAO 把尺寸设为 order-4（64 KB）并保留 15% 物理内存给 mTHP-only 分配后，成功率从「2 小时后 <10%」稳定回到 **50% 以上**；双 LRU（base page 一条、large folio 一条）维持大页可得性 [LWN-mTHP, OPPO-LPC]。
...
- **zRAM 一刀切压缩** —— Ariadne 区分热（不压、留 DRAM）/温（压入 zpool）/冷（外移 flash），并对冷数据用大压缩块、热数据用小块，**相关重启延迟降低 50%、压缩/解压 CPU 降低 15%** [Ariadne]。

- **冷启动无预取/回收协同** —— AppFlow 利用「97.2% 冷启动 I/O 可预测、79% DRAM I/O 带宽闲置」，做内存调度与预取，**冷启动延迟最多降 66.5%、直接回收减 67.9%、LMK 事件减 33.7%、多保留 1.85× 后台 App** [AppFlow]。

### 尚未解决的问题

- **缺页/回收的 eBPF 挂钩尚未进 mainline** —— eBPF-mm、cachebpf、FetchBPF、PageFlex 截至 2025 年中多为研究原型，缺页与回收挂钩未合入主线；端侧目前真正可用的可编程面主要是 mainline 的 DAMOS 与 userspace LMKD（PSI），尚非任意 eBPF 程序 [eBPF-mm, PageFlex, LSFMMBPF]。

- **采样驱动策略需要负载稳定性** —— DAMON 采样在阶段切换或突发负载下需持续重采样，手机的前后台频繁切换正是这种突发场景，可能抵消部分收益 [DAMON-Reclaim]。

- **eBPF 验证器约束限制策略复杂度** —— BPF 验证器强制有界循环与有限栈深，端侧若要在策略里跑复杂热度预测或 ML 难以直接实现，需借助 map / BPF-to-BPF 等绕行。

- **多策略点相互干扰且缺乏功耗预算数据** —— 缺页选尺寸、DAMOS 回收、zRAM 压缩、LMKD 查杀同时生效时可能病态交互（如刚提升的大页被立即拆分回收）；且端侧最敏感的**持续采样/压缩/预取功耗**尚无公开的电池影响数据。
...
**Adaptive（自适应）** —— 端侧内存管理从编译时固定、系统级一刀切的策略，转变为按访问热度、按设备碎片状态、按应用前后台状态在运行时自我调整的策略：mTHP 让大页成功率在运行 2 小时后从 <10% 回到 >50%，DAMOS 以 <1% 单核采样开销节省 32% 内存，自适应查杀/压缩把 OOM 查杀降低 6.5× 并把相关重启延迟降低 50%。其实现机制是「可编程（Programmable）」——把策略面开放给运行时可加载的程序，但端侧的价值落点是自适应。

## 8. 开放问题与注意事项

- **可编程内存挂钩的端侧上游路径** —— DAMOS 已在 mainline，但缺页选尺寸、页缓存淘汰、回收的通用 eBPF 挂钩尚未合入；Android GKI 何时、以何种受限形式开放这些策略面仍不明确 [LSFMMBPF, eBPF-mm]。
- **自适应策略的功耗与续航代价** —— 持续 DAMON 采样、热冷分离压缩、冷启动预取都消耗 CPU/IO；现有论文几乎都报告吞吐/延迟收益，却罕见在真实电池设备上的**持续功耗与发热**数据。
- **碎片化的长期治理** —— TAO 的保留区/双 LRU 在数小时尺度有效，但连续运行数天、跨多次前后台切换后的大页可得性仍缺长程实测 [OPPO-LPC]。
- **多策略点协调** —— 选尺寸、回收、压缩、查杀、预取各自自适应时缺乏统一协调；端侧尚无公开工作处理「多挂钩/多策略联合优化」以避免相互拆台。
- **跨设备与跨厂商可移植性** —— 现有数字多来自特定机型（Pixel 7、OPPO 量产机），低端 eMMC + 小内存设备上的表现可能差距更大；mTHP/DAMOS 配置如何随设备档位自动适配尚无标准。
- **基准代表性** —— 端侧负载（相机、游戏、本地大模型、后台同步）访问模式差异巨大，多数评测仅覆盖少数 App，生产环境的长尾负载仍待验证。
